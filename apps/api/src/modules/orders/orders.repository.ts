...
export class OrdersRepository {
      ...
      if (client) {
        const result = await client.query<IOrderRow>(
          `INSERT INTO option_orders (
             ...
             product_type, quantity, price, trigger_price, trailing_stop_loss, average_price, status,
  ...
  public async getOrdersByUserId(userId: string, status?: string): Promise<OptionOrderEntity[]> {
    try {
      let query = `
        SELECT id, client_order_id, user_id, contract_id, order_type, transaction_type,
               product_type, quantity, price, trigger_price, average_price, status,
               rejection_reason, executed_at, created_at, updated_at
        FROM option_orders
        WHERE user_id = $1
      `;
    ...
    const orders = Array.from(OrdersRepository.memOrders.values()).filter(
  ...
  public async updateOrder(
        ...
        const result = await client.query<IOrderRow>(
          `UPDATE option_orders
           SET price = COALESCE($2, price),
               ...
               trailing_stop_loss = COALESCE($4, trailing_stop_loss),
    ...
    const updated: OptionOrderEntity = {
      ...
      triggerPrice: updates.triggerPrice ?? existing.triggerPrice,
      trailingStopLoss: updates.trailingStopLoss ?? existing.trailingStopLoss,
